...
type TopPlacesToVisitProps = {
  topPlacesToVisit: Plan["topplacestovisit"] | undefined;
  planId: string;
  isLoading: boolean;
  allowEdit: boolean;
};

type TopPlacesWithIdType = (Plan["topplacestovisit"][number] & {id: string})[];
...
const TopPlacesToVisit = ({
  ...
  const addNewPlaceToTopPlaces = (lat: number, lng: number, placeName: string) => {
    ...
    // Without this, the new pin is added to the map's data but the map's
    // viewport never pans/zooms to it (Map.tsx only recenters in response to
    // selectedPlace changing) - looks like the map "isn't updating".
  ...
  const handleDeletPlace = (id: string) => {
    ...
    updatePartOfPlan(planId, "topplacestovisit", copy)
      .then(() => {
        dismiss();
        setTopPlaces((places) => {
