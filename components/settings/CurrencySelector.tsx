...
import { addPreferredCurrency as addPreferredCurrencyRequest } from "@/lib/client/plan";
...
const CurrencySelector = ({ planId }: { planId: string }) => {
  ...
  const { data: preferredCurrency } = useSWR(
    `/api/plans/${planId}/settings/currency`,
    fetcher
  );
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    try {
      await addPreferredCurrencyRequest(planId, values.currency);
      form.reset();
      toast({
        description: (
            ...
            render={({ field }) => (
              <FormItem>
                <FormControl>
                  ...
                  >
                    <SelectTrigger className="max-w-md" aria-label="Preferred Currency">
                      <SelectValue
                        placeholder={
