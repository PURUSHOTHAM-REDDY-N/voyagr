...
import WeatherSuitabilityAlert from "@/components/plan/WeatherSuitabilityAlert";
import { Button } from "@/components/ui/button";
import { useEffect, useState } from "react";
import { useItineraryWeatherForecast } from "@/hooks/useItineraryWeatherForecast";
...
const Plan = ({ planId }: PlanProps) => {
  ...
  const { forecast, conflicts } = useItineraryWeatherForecast(
    planId,
    !isLoading && !!plan?.contentGenerationState.itinerary
  );
  ...
  return (
    <section className="h-full flex flex-col gap-5">
      ...
      <WeatherSuitabilityAlert conflicts={conflicts} />
      <PlanMetaData
        allowEdit={true}
        companionId={plan?.companion ?? undefined}
