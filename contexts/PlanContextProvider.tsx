"use client";
import type { Plan, ContentGenerationState, PlanSettings } from "@prisma/client";
...
type planStateType = ContentGenerationState & {
  weather: boolean;
};
...
type PlanContextType = {
  ...
  // fromDate/toDate come back from the API as epoch-millis numbers (see
  // lib/server/serialize.ts), not Prisma's PlanSettings' native Date type.
  plan:
    | (Plan & {
        ...
        fromDate?: number;
        toDate?: number;
        isPublished: PlanSettings["isPublished"];
      })
    | null
    | undefined;
  isLoading: boolean;
